<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
   ...
   * @return string
   */
  function dataAtual( $strFormato = "Y-m-d" )
  {
  ...
  */
   ...
   * @return string
   */
  function adicionarDias( $data, $quantidade, $strIntervalo = "d", $bolRetornaDiaUtil = false, $bolSabadoDiaUtil = false )
  {
   ...
   * @return string
   */
  function adicionarHoras( $hora, $quantidade, $strIntervalo = "h", $bolRetornaSegundos = false )
  {
   ...
   * @return string
   */
  function adicionarDiasUteis( $data, $dias, $bolSabadoDiaUtil = false )
  {
   ...
   * @return string
   */
  function calcularDiferencaDias( $strDataInicio, $strDataFim, $strIntervalo = "d", $bolDiasUteis = false, $bolSabadoDiaUtil = false )
  {
    ...
    $strDataInicio = Padrao2Banco( $strDataInicio );
    $strDataFim = Padrao2Banco( $strDataFim );
    ...
    {
      $intBissextos = intval( calcularAnosBissextos( $strDataInicio, $strDataFim ) );
   ...
   * @return string
   */
  function calcularDiferencaTempo( $strDataInicio, $strDataFim )
  {

    $diferenca = calcularDiferencaDias( $strDataInicio, $strDataFim, "s" );
   ...
   * @return string
   */
  function calcularIdade( $data_nascimento, $strDataAtual = null )
  {

    if ( strlen( $data_nascimento ) > 0 )
    {

      $data_nascimento = Padrao2Banco( $data_nascimento );


      $data_atual = (strlen( $strDataAtual ) > 0) ? Padrao2Banco( $strDataAtual ) : dataAtual( "Y-m-d" );
   ...
   * @return string
   */
  function calcularAnosBissextos( $strDataInicio, $strDataFim )
  {

    $data_inicio = Padrao2Banco( $strDataInicio );

    $data_inicio = explode( "-", $data_inicio );
    $ano_inicio = $data_inicio['0'];


    $data_fim = Padrao2Banco( $strDataFim );
   ...
   * @return string
   */
  function retornaNomeMes( $intMes )
  {
   ...
   * @return string
   */
  function retornaNomeDataCompacta( $data )
  {

    $data = Padrao2Banco( $data );

    $nomeMes = retornaNomeMes( $data );
   ...
   * @return string
   */
  function exibirDataPorExtenso( $data = null, $bolExibirDiaDaSemana = true )
  {
   ...
   * @return string
   */
  function exibirMensagemCumprimento()
  {
    $hora_do_dia = date( "H" );
   ...
   * @return boolean
   */
  function verificaDiaUtil( $data, $bolSabadoDiaUtil = false )
  {
